resource "aws_security_group" "security_group_public" {
  name        = "${var.prefix}-public-sequrity-group"
  description = "Allow ICMP, SSH, and port 8080"
...
resource "aws_security_group" "security_group_private" {
  name        = "${var.prefix}-private-sequrity-group"
  description = "Allow ICMP, SSH, and port 8080 from public subnet only"
  vpc_id      = aws_vpc.vpc_main.id
